...
def _numerical_gradient(
    fn: SigmoidLayer | ReluLayer | TanhLayer | IdentityLayer,
    x: npt.NDArray[np.float64],
    eps: float = 1e-5,
) -> npt.NDArray[np.float64]:
    """Compute numerical gradient via central difference for activation layers."""
    ...
    for idx in np.ndindex(x.shape):
        ...
        x_plus[idx] += eps
        x_minus[idx] -= eps
        grad[idx] = (np.sum(fn.forward(x_plus)) - np.sum(fn.forward(x_minus))) / (2 * eps)
    return grad
...
class TestSigmoidLayer:
    ...
    def test_backward_matches_numerical(self) -> None:
        ...
        numerical_grad = _numerical_gradient(layer, x)
