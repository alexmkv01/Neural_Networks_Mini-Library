...
class LinearLayer(Layer):
    ...
    def __init__(self, n_in: int, n_out: int) -> None:
        self.n_in = n_in
        self.n_out = n_out
        self._weights: npt.NDArray[np.float64] = xavier_init(n_in, n_out)
        self._biases: npt.NDArray[np.float64] = zeros_init(n_in, n_out)
    ...
    def update_params(self, learning_rate: float) -> None:
        if self._grad_weights is None or self._grad_biases is None:
            raise ValueError("backward() must be called before update_params()")
